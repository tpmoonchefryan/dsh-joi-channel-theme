...
> [!NOTE]
> **命令行只用这一次。** 装完之后所有操作都在界面里：换装在 设置 → 通用设置，
> 总开关在 设置 → 插件 → 插件配置。DeepSeek Harness 目前没有图形化的插件安装入口，
> 所以这一步绕不开。
...
**总开关**在 设置 → 插件 → 插件配置：关掉即回到 DeepSeek 原生外观，插件保持安装；
再打开会回到你关掉前那套衣装。装了插件，不等于必须换肤。
